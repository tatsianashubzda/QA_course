...
// Var.1
let findUserFromData = function (d1, d2) {

    for (let i = 0; i < users.length; i++) {
        if (users[i].registrationDate == d1 || users[i].registrationDate == d2) console.log(users[i].firstName + ' ' + users[i].lastName)
    }
}
findUserFromData(data1, data2)


// Var.2
users.forEach(item => { if (item.registrationDate == data1 || item.registrationDate == data2) console.log(item.firstName + ' ' + item.lastName); })